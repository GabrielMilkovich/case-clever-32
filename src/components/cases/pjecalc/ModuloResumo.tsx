  ...
  const executarLiquidacao = async () => {
    ...
    try {
      ...
      const fgtsConfig: PjeFGTSConfig = {
        apurar: fgtsRes.data?.apurar ?? true,
        destino: fgtsRes.data?.destino || 'pagar_reclamante',
        compor_principal: fgtsRes.data?.compor_principal ?? true,
        multa_apurar: fgtsRes.data?.multa_apurar ?? true,
        multa_tipo: fgtsRes.data?.multa_tipo || 'calculada',
        multa_percentual: fgtsRes.data?.multa_percentual ?? 40,
        multa_base: fgtsRes.data?.multa_base || 'devido',
        multa_valor_informado: fgtsRes.data?.multa_valor_informado,
        saldos_saques: fgtsRes.data?.saldos_saques || [],
        deduzir_saldo: fgtsRes.data?.deduzir_saldo ?? false,
        lc110_10: fgtsRes.data?.lc110_10 ?? false,
        lc110_05: fgtsRes.data?.lc110_05 ?? false,
      };

      const csConfig: PjeCSConfig = {
        apurar_segurado: csRes.data?.apurar_segurado ?? true,
        cobrar_reclamante: csRes.data?.cobrar_reclamante ?? true,
        cs_sobre_salarios_pagos: csRes.data?.cs_sobre_salarios_pagos ?? false,
        aliquota_segurado_tipo: csRes.data?.aliquota_segurado_tipo || 'empregado',
        aliquota_segurado_fixa: csRes.data?.aliquota_segurado_fixa,
        limitar_teto: csRes.data?.limitar_teto ?? true,
        apurar_empresa: csRes.data?.apurar_empresa ?? true,
        apurar_sat: csRes.data?.apurar_sat ?? true,
        apurar_terceiros: csRes.data?.apurar_terceiros ?? true,
        aliquota_empregador_tipo: 'fixa',
        aliquota_empresa_fixa: csRes.data?.aliquota_empresa_fixa ?? 20,
        aliquota_sat_fixa: csRes.data?.aliquota_sat_fixa ?? 2,
        aliquota_terceiros_fixa: csRes.data?.aliquota_terceiros_fixa ?? 5.8,
        periodos_simples: csRes.data?.periodos_simples || [],
      };

      const irConfig: PjeIRConfig = {
        apurar: irRes.data?.apurar ?? true,
        incidir_sobre_juros: irRes.data?.incidir_sobre_juros ?? false,
        cobrar_reclamado: irRes.data?.cobrar_reclamado ?? false,
        tributacao_exclusiva_13: irRes.data?.tributacao_exclusiva_13 ?? true,
        tributacao_separada_ferias: irRes.data?.tributacao_separada_ferias ?? false,
        deduzir_cs: irRes.data?.deduzir_cs ?? true,
        deduzir_prev_privada: irRes.data?.deduzir_prev_privada ?? false,
        deduzir_pensao: irRes.data?.deduzir_pensao ?? false,
        deduzir_honorarios: irRes.data?.deduzir_honorarios ?? false,
        aposentado_65: irRes.data?.aposentado_65 ?? false,
        dependentes: irRes.data?.dependentes ?? 0,
      };

      const correcaoConfig: PjeCorrecaoConfig = {
        indice: correcaoRes.data?.indice || 'IPCA-E',
        epoca: correcaoRes.data?.epoca || 'mensal',
        data_fixa: correcaoRes.data?.data_fixa,
        juros_tipo: correcaoRes.data?.juros_tipo || 'selic',
        juros_percentual: correcaoRes.data?.juros_percentual ?? 1,
        juros_inicio: correcaoRes.data?.juros_inicio || 'ajuizamento',
        multa_523: correcaoRes.data?.multa_523 ?? false,
        multa_523_percentual: correcaoRes.data?.multa_523_percentual ?? 10,
        data_liquidacao: correcaoRes.data?.data_liquidacao || new Date().toISOString().slice(0, 10),
      };
