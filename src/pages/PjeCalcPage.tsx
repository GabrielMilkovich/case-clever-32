...
export default function PjeCalcPage() {
  ...
  // Auto-fill from contract or existing params
  useEffect(() => {
    if (params) {
      setFormParams({
  ...
  const saveParams = async () => {
    ...
    try {
      const payload = {
        ...
        data_demissao: formParams.data_demissao || null,
        data_ajuizamento: formParams.data_ajuizamento,
        data_inicial: formParams.data_inicial || null,
        data_final: formParams.data_final || null,
  ...
  const renderParametros = () => (
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div>
            ...
            <Input type="date" value={formParams.data_admissao} onChange={e => setFormParams(p => ({ ...p, data_admissao: e.target.value }))} className="mt-1 h-8 text-xs" />
          </div>
          <div>
            <Label className="text-xs">Demissão</Label>
            <Input type="date" value={formParams.data_demissao} onChange={e => setFormParams(p => ({ ...p, data_demissao: e.target.value }))} className="mt-1 h-8 text-xs" />
          </div>
          <div>
            <Label className="text-xs">Ajuizamento *</Label>
            <Input type="date" value={formParams.data_ajuizamento} onChange={e => setFormParams(p => ({ ...p, data_ajuizamento: e.target.value }))} className="mt-1 h-8 text-xs" />
          </div>
          <div>
            <Label className="text-xs">Data Inicial</Label>
            <Input type="date" value={formParams.data_inicial} onChange={e => setFormParams(p => ({ ...p, data_inicial: e.target.value }))} className="mt-1 h-8 text-xs" />
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          <div className="flex items-center gap-2">
            <Checkbox checked={formParams.prescricao_quinquenal} onCheckedChange={v => setFormParams(p => ({ ...p, prescricao_quinquenal: !!v }))} />
            <Label className="text-xs">Aplicar Prescrição Quinquenal</Label>
          </div>
          <div className="flex items-center gap-2">
            <Checkbox checked={formParams.prescricao_fgts} onCheckedChange={v => setFormParams(p => ({ ...p, prescricao_fgts: !!v }))} />
            <Label className="text-xs">Aplicar Prescrição FGTS</Label>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <Input type="number" value={formParams.carga_horaria_padrao} onChange={e => setFormParams(p => ({ ...p, carga_horaria_padrao: parseInt(e.target.value) || 220 }))} className="mt-1 h-8 text-xs" />
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="grid grid-cols-2 gap-4">
          <div>
            <Label className="text-xs">Maior Remuneração (R$)</Label>
            <Input type="number" step="0.01" value={formParams.maior_remuneracao} onChange={e => setFormParams(p => ({ ...p, maior_remuneracao: e.target.value }))} className="mt-1 h-8 text-xs" placeholder="0,00" />
          </div>
          <div>
            <Label className="text-xs">Última Remuneração (R$)</Label>
            <Input type="number" step="0.01" value={formParams.ultima_remuneracao} onChange={e => setFormParams(p => ({ ...p, ultima_remuneracao: e.target.value }))} className="mt-1 h-8 text-xs" placeholder="0,00" />
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent className="grid grid-cols-2 gap-4">
          ...
          {formParams.prazo_aviso_previo === 'informado' && (
            <div>
              ...
              <Input type="number" value={formParams.prazo_aviso_dias} onChange={e => setFormParams(p => ({ ...p, prazo_aviso_dias: e.target.value }))} className="mt-1 h-8 text-xs" />
            </div>
          )}
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          <div className="flex items-center gap-2">
            <Checkbox checked={formParams.projetar_aviso_indenizado} onCheckedChange={v => setFormParams(p => ({ ...p, projetar_aviso_indenizado: !!v }))} />
            <Label className="text-xs">Projetar Aviso Prévio Indenizado</Label>
          </div>
          <div className="flex items-center gap-2">
            <Checkbox checked={formParams.limitar_avos_periodo} onCheckedChange={v => setFormParams(p => ({ ...p, limitar_avos_periodo: !!v }))} />
            <Label className="text-xs">Limitar Avos ao Período do Cálculo</Label>
          </div>
          <div className="flex items-center gap-2">
            <Checkbox checked={formParams.zerar_valor_negativo} onCheckedChange={v => setFormParams(p => ({ ...p, zerar_valor_negativo: !!v }))} />
            <Label className="text-xs">Zerar Valor Negativo</Label>
          </div>
          <div className="flex items-center gap-2">
            <Checkbox checked={formParams.sabado_dia_util} onCheckedChange={v => setFormParams(p => ({ ...p, sabado_dia_util: !!v }))} />
            <Label className="text-xs">Sábado como Dia Útil</Label>
          </div>
          <div className="flex items-center gap-2">
            <Checkbox checked={formParams.considerar_feriado_estadual} onCheckedChange={v => setFormParams(p => ({ ...p, considerar_feriado_estadual: !!v }))} />
            <Label className="text-xs">Considerar Feriado Estadual</Label>
          </div>
          <div className="flex items-center gap-2">
            <Checkbox checked={formParams.considerar_feriado_municipal} onCheckedChange={v => setFormParams(p => ({ ...p, considerar_feriado_municipal: !!v }))} />
            <Label className="text-xs">Considerar Feriado Municipal</Label>
          </div>
        </CardContent>
      </Card>
    </div>
      ...
      <div className="flex items-center justify-between">
        ...
        <Button size="sm" onClick={async () => {
          await supabase.from("pjecalc_faltas").insert({
            case_id: caseId!, data_inicial: new Date().toISOString().slice(0, 10),
            data_final: new Date().toISOString().slice(0, 10), justificada: false,
          });
          queryClient.invalidateQueries({ queryKey: ["pjecalc_faltas", caseId] });
        }}>
          <Plus className="h-4 w-4 mr-1" /> Nova Falta
        </Button>
      </div>
      <p className="text-xs text-muted-foreground">Informe todas as faltas durante o contrato, mesmo anteriores ao período de cálculo.</p>
      {faltas.length === 0 ? (
            ...
            <Card key={f.id}>
              <CardContent className="p-3 flex items-center gap-3">
                ...
                <div className="flex items-center gap-1">
                  <Checkbox defaultChecked={f.justificada} onCheckedChange={v => supabase.from("pjecalc_faltas").update({ justificada: !!v }).eq("id", f.id)} />
                  <Label className="text-xs">Justificada</Label>
                </div>
                <Button variant="ghost" size="icon" className="h-7 w-7 ml-auto" onClick={async () => {
                  await supabase.from("pjecalc_faltas").delete().eq("id", f.id);
                  queryClient.invalidateQueries({ queryKey: ["pjecalc_faltas", caseId] });
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Button size="sm" variant="outline" onClick={async () => {
          if (!formParams.data_admissao || !formParams.data_demissao) {
            toast.error("Preencha as datas de admissão e demissão nos Parâmetros.");
            return;
          }
          // Auto-gerar períodos aquisitivos
          await supabase.from("pjecalc_ferias").delete().eq("case_id", caseId!);
          const adm = new Date(formParams.data_admissao);
          const dem = new Date(formParams.data_demissao);
          const periodos = [];
          ...
          while (aqInicio < dem) {
            ...
            periodos.push({
              ...
              prazo_dias: formParams.regime_trabalho === 'tempo_integral' ? 30 : 18,
              situacao,
              dobra: situacao === 'indenizadas' ? false : (concFim > dem),
            });
          ...
          toast.success(`${periodos.length} período(s) de férias gerado(s)`);
        }}>
          <Calculator className="h-4 w-4 mr-1" /> Gerar Automaticamente
        </Button>
      </div>

      {ferias.length === 0 ? (
        <Card><CardContent className="p-8 text-center text-sm text-muted-foreground">
          Clique em "Gerar Automaticamente" para criar os períodos de férias com base nas datas do contrato.
        </CardContent></Card>
      ) : (
          ...
          <table className="w-full text-xs border-collapse">
            <thead>
              <tr className="bg-muted/50">
                <th className="p-2 text-left font-medium">Relativas</th>
                <th className="p-2 text-left font-medium">P. Aquisitivo</th>
                <th className="p-2 text-left font-medium">P. Concessivo</th>
                <th className="p-2 text-center font-medium">Prazo</th>
                <th className="p-2 text-center font-medium">Situação</th>
                <th className="p-2 text-center font-medium">Dobra</th>
                <th className="p-2 text-center font-medium">Abono</th>
              </tr>
            </thead>
            <tbody>
              {ferias.map((f: any) => (
                <tr key={f.id} className="border-b border-border/50 hover:bg-muted/20">
                  ...
                  <td className="p-2 text-center">
                    <Select defaultValue={f.situacao} onValueChange={async v => {
                      await supabase.from("pjecalc_ferias").update({ situacao: v }).eq("id", f.id);
                      queryClient.invalidateQueries({ queryKey: ["pjecalc_ferias", caseId] });
                    }}>
                  ...
                  <td className="p-2 text-center">
                    <Checkbox defaultChecked={f.dobra} onCheckedChange={v => supabase.from("pjecalc_ferias").update({ dobra: !!v }).eq("id", f.id)} />
                  </td>
                  <td className="p-2 text-center">
                    <Checkbox defaultChecked={f.abono} onCheckedChange={v => supabase.from("pjecalc_ferias").update({ abono: !!v }).eq("id", f.id)} />
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <div className="flex items-center justify-between">
        ...
        <Button size="sm" onClick={async () => {
          ...
          await supabase.from("pjecalc_historico_salarial").insert({
            case_id: caseId!, nome: `Salário Base ${historicos.length + 1}`,
            periodo_inicio: formParams.data_admissao, periodo_fim: formParams.data_demissao || new Date().toISOString().slice(0, 10),
            tipo_valor: 'informado', incidencia_fgts: true, incidencia_cs: true,
          });
          queryClient.invalidateQueries({ queryKey: ["pjecalc_historico", caseId] });
        }}>
          <Plus className="h-4 w-4 mr-1" /> Nova Base
        </Button>
      </div>
      <p className="text-xs text-muted-foreground">Cadastre as bases de cálculo que serão usadas nas verbas, FGTS e contribuições.</p>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                ...
                <Button variant="ghost" size="icon" className="h-7 w-7" onClick={async () => {
                  await supabase.from("pjecalc_historico_salarial").delete().eq("id", h.id);
                  queryClient.invalidateQueries({ queryKey: ["pjecalc_historico", caseId] });
                }}><Trash2 className="h-3 w-3" /></Button>
              </div>
            </div>
  ...
  const renderVerbas = () => (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button size="sm" variant="outline" onClick={async () => {
            // Lançamento Expresso: adicionar verbas comuns
            ...
            for (const ve of verbasExpresso) {
              await supabase.from("pjecalc_verbas").insert({
                case_id: caseId!, nome: ve.nome, tipo: ve.tipo,
                caracteristica: ve.caracteristica, ocorrencia_pagamento: ve.ocorrencia_pagamento,
                multiplicador: ve.multiplicador, divisor_informado: ve.divisor_informado || 30,
            ...
            <Briefcase className="h-4 w-4 mr-1" /> Expresso
          </Button>
          <Button size="sm" onClick={async () => {
            const periodo = formParams.data_admissao && formParams.data_demissao ? {
              inicio: formParams.data_admissao, fim: formParams.data_demissao,
      ...
      {verbas.length === 0 ? (
        <Card><CardContent className="p-8 text-center text-sm text-muted-foreground">
          Use "Expresso" para incluir verbas comuns ou "Manual" para criar uma verba personalizada.
        </CardContent></Card>
      ) : (
        <div className="space-y-2">
          {verbas.map((v: any) => (
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-[10px] text-muted-foreground flex gap-2">
                        ...
                        <span>•</span>
                        <span>×{v.multiplicador} ÷{v.divisor_informado || 30}</span>
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
